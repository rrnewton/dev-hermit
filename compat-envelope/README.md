...
- **bitwise** — cells with a dereferenced cross-backend `BitwiseInfoV1` witness:
  ...
  declared first-appearance-ordinal policy. Stack and heap evidence is computed
  over a backend-neutral guest-state projection: snapshot producers mark
  instrumentation-owned injected regions, and the shared comparator excludes
  those regions by provenance before hashing. Backend names and hardcoded address
  or region lists are not part of the policy; every remaining guest byte must
  match exactly.
...
The stricter contract will likely reduce the legacy stdout-equality percentages
when first measured. That is corrected classification, not a regression: the old
number credited cells without testing the required INFO, stack, and heap signals.
