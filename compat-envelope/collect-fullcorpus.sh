...
measure() { # $1=backend $2=cell-dir(holds ptv.out ref) $3=lane $4=id ; rest=guest argv
  ...
  if [ "$backend" = ptrace ]; then
    ...
    echo "fullcorpus,$RUN_UTC,$HSHA,$RSHA,false,expansion,$lane,$bucket,$id,verify,ptrace,expansion,$outcome,$det,,$ohash,$dur,,$reason,$verify_compare,,,$det_tier,,$ohash,,,,$relaxation_csv,,,,,unqualified-stdout-only" > "$ROWS/ptrace_${id//\//_}.row"
    return
  fi
  ...
  echo "fullcorpus,$RUN_UTC,$HSHA,$RSHA,false,expansion,$lane,$bucket,$id,verify,$backend,expansion,$outcome,$det,,$ohash,$dur,,$reason,$verify_compare,,,$det_tier,$legacy_parity,$ref_hash,,,,$relaxation_csv,,,,,unqualified-stdout-only" > "$ROWS/${backend}_${id//\//_}.row"
}
