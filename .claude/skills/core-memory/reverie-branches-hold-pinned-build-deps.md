---
name: core-memory-reverie-branches-hold-pinned-build-deps
description: "Deleting rrnewton/reverie branches can orphan a hermit-pinned build dep; check containment first (CORE-MEMORY mirror of memory/reverie-branches-hold-pinned-build-deps.md)"
---

# CORE-MEMORY: reverie-branches-hold-pinned-build-deps

<!-- GENERATED MIRROR of core memory `reverie-branches-hold-pinned-build-deps`. Source of truth is the memory
     file `reverie-branches-hold-pinned-build-deps.md`. Regenerate: scripts/sync-memory-skill.rs. Verify in
     sync: scripts/lint-memory-skill-sync.rs. Do NOT hand-edit inside the
     markers — edit the memory and re-run sync. -->

<!-- BEGIN CORE-MEMORY-MIRROR (source: reverie-branches-hold-pinned-build-deps.md) -->
Before deleting branches on rrnewton/reverie, check every reverie `?rev=SHA` that hermit pins (grep `reverie.git?rev=` across all `~/work/dev-hermit` Cargo.lock/Cargo.toml) against branch containment (`git branch -r --contains <sha>` in a reverie checkout). Hermit pins reverie by exact SHA, not by branch name, so a "stale" feature branch can be the ONLY ref keeping a live pinned commit reachable — deleting it orphans the commit and breaks `cargo` fresh builds (GitHub won't serve an unreachable SHA).
