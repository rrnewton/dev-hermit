...
row. `ci-hub validate-status --sha <head>` is the landing verifier; the raw exit
code and `locally-validated` label are not.
...
green result. See [validation orchestration](validate-orchestrator-discipline.md).
