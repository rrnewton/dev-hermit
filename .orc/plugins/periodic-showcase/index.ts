...
// ---------------------------------------------------------------------------
// RETIRED RUNTIME WORKFLOWS.
//
// Two predecessors existed only as RUNTIME registrations -- nothing in version
// control declared either -- and both are superseded by the workflow above.
// Retiring them HERE rather than by hand is what makes the retirement durable:
// a hand-killed runtime workflow leaves no record of why it went, and nothing
// stops the next session from recreating it.
//
// Diagnosed live on 2026-08-07 from the engine's own persisted state
// (`orc curl /api/workflows`):
//
//   demo-run-owner-pitch-watch — UNREPAIRABLE, and the reason is not subtle:
//     its persisted `source_text` is the 26-character string
//     "demo-run-owner-pitch-watch", i.e. the workflow NAME was stored where the
//     source belongs. There is no function to fix. Every restart re-evaluates
//     it as the expression `demo - run - owner - pitch - watch` and throws
//     ReferenceError, surfaced by the engine as "evaluate workflow source:
//     Exception generated by QuickJS" ~4ms after start. That is exactly why a
//     blind restart crashed it a second time.
//
//   demo-presentation — VALID source (2236 chars, already wf.sleep/wf.loop
//     shaped) and NOT the crash. It is retired on policy, not on evaluation:
//     it hardcoded `sendWakeup(["orc"], ...)`, titled its output "DEMO
//     PRESENTATION RUN" against the `Periodic showcase` naming policy, and
//     fired unconditionally every 3h — the forced novelty the owner rejected.
//
// TIMING IS THE POINT. This runs in the same plugin load that registers the
// replacement, so the retirement and the live replacement land together and
// there is never a window with neither. Killing them ahead of that would have
// satisfied "workflow absent" while leaving the responsibility uncovered.
//
// Precedent: hermit-dev/index.ts retires LEGACY_PR_HEALTH_WORKFLOW_NAME from
// its own startup hook in exactly this way.
const RETIRED_WORKFLOW_NAMES = [
  "demo-presentation",
  "demo-run-owner-pitch-watch",
];
...
function registerPluginSurface(): void {
  ...
  // Retire the superseded runtime workflows, in the same load that registered
  // the replacement above. Deliberately LAST: if anything before it failed, the
  // replacement is not live and retiring would open a coverage gap.
  //
  // `registerStartup` is used rather than doing this inline because killWorkflow
  // is async and module scope cannot await. It is absent from
  // orc.listEffects() on this build yet works (hermit-dev documents the same),
  // so it is called directly rather than probed -- a probe would report a false
  // negative and skip the retirement.
  orc.registerStartup(SHOWCASE_PLUGIN_NAME + ".retire", async function () {
    for (const name of RETIRED_WORKFLOW_NAMES) {
      try {
        await orc.killWorkflow(name);
        orc.log("info", SHOWCASE_PLUGIN_NAME + ": retired superseded workflow " + name);
      } catch (_err) {
        // Already absent in a fresh session, which is the steady state after
        // the first load. Not an error, and never fatal: a failure to retire an
        // obsolete workflow must not take down the replacement that just
        // registered.
      }
    }
  });
