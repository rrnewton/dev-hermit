...
`libunwind-x86_64.so.8 => not found` without it), but that variable is inherited
by the guest, whose loader then searches those directories -- and every probe of
them lands in the DETLOG as an absolute host path. Measured on /bin/true:
