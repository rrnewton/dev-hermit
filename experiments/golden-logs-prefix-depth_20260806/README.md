...
Build and runtime use **different** libunwind directories:

| var | dir | why |
|---|---|---|
| `PKG_CONFIG_PATH` | `ignored/lu-parity/usr/lib64/pkgconfig` | build script |
| `LIBRARY_PATH` | `ignored/lu-parity/usr/lib64` | **link** search; omitting it fails with `rust-lld: unable to find library`, which reads as a code error and is not one |
| `LD_LIBRARY_PATH` | `/home/newton/fbsource/.../libunwind/lib` | **runtime**; hermit will not start without it |

Use the fbsource dir at runtime. Both dirs carry `libunwind-x86_64.so.8`, so
hermit starts under either and these captures were valid under lu-parity — but
lu-parity ships libunwind-ptrace only as a static `.a`, so anything needing
`libunwind-ptrace.so.0` fails there in a way that looks like a broken build.
Verified the choice does **not** move `Z`: `/bin/true` 14 and `/bin/echo hello`
43 under either dir.
