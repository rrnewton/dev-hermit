...
const ALL_BACKENDS: [Backend; 7] = [
    ...
    Backend {
        name: "gvisor-systrap",
        kind: BackendKind::Gvisor("counter1-systrap"),
    },
    Backend {
        name: "gvisor-kvm",
        kind: BackendKind::Gvisor("counter1-kvm"),
    },
    Backend {
        name: "reverie-ptrace",
...
impl Paths {
    fn discover() -> Result<Self> {
        ...
        Ok(Self {
            ...
            counter1: env_path("COUNTER1")
                .unwrap_or_else(|| reverie_slot.join("target/release/counter1")),
            ...
            kvm_counter1: env_path("KVM_COUNTER1")
                .unwrap_or_else(|| reverie_slot.join("target/release/reverie-kvm-counter1")),
    ...
    fn command(self, paths: &Paths, socket: Option<&Path>) -> Result<(Command, Option<TempDir>)> {
        match self.kind {
            ...
            BackendKind::ReveriePtrace => {
                require_executable(&paths.counter1, "Reverie counter1")?;
                let mut command = Command::new(&paths.counter1);
                command.arg("--");
            ...
            BackendKind::ReverieDbi => {
                ...
                command
                    ...
                    .env("HERMIT_DBI_COUNTER1_EXACT", "1")
                    .arg("-summary")
                    .arg("--");
                append_helper(&mut command, paths, socket);
                Ok((command, None))
            ...
            BackendKind::ReverieKvm => {
                require_executable(&paths.kvm_counter1, "Reverie KVM counter")?;
                let mut command = Command::new(&paths.kvm_counter1);
                append_helper(&mut command, paths, socket);
                Ok((command, None))
            }
...
fn run() -> Result<()> {
    ...
    eprintln!("counter1: {}", paths.counter1.display());
    eprintln!("drrun: {}", paths.drrun.display());
    eprintln!("DBI client: {}", paths.dbi_client.display());
    eprintln!("KVM counter: {}", paths.kvm_counter1.display());
