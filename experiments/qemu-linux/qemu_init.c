/*
 * Freestanding static /init for the hermit+QEMU deterministic-compat profile.
 *
 * Prints one marker line with the kernel release/machine, syncs, and issues the
 * Linux reboot(POWER_OFF) syscall so the guest exits cleanly. No libc: built
 * with -nostdlib -static so the initramfs is a single tiny executable.
 *
 ...
 *   hermit/experiments/shared-futex-verify_20260722/qemu_init.c
 * to keep this experiment self-contained.
 */
enum {
  SYS_WRITE = 1,
  SYS_PAUSE = 34,
