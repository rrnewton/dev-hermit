...
- **Headline:** every package tested diverges natively when built in two
  different roots; under `hermit run --strict --no-rcb-time` every package
  measured in that arm is byte-identical. Exact counts are in the generated
  headline below the table, and regenerate with `./regen_readme.py`.
- **Each package is its own control.** The native pair establishes per package
  that the build really was root-sensitive, so no win is vacuous.
- **This measures one of two mechanisms.** The design holds the *guest-visible*
  build path constant and varies only the *host* root, so it isolates
  path-*triggered* divergence. A separate probe varies the guest path and shows
  Hermit correctly does **not** mask that — both mechanisms reproduced, with
  opposite outcomes, as predicted.
- **One arm is confounded and labelled as such.** This host's PMU fails
  validation, and Hermit derives virtual time from branch counts, so the arm
  without `--no-rcb-time` measures the broken PMU as much as it measures Hermit.
- **The sample is convenience-biased** toward small, fast-building packages and
  must not be reported as a percentage of the 8,688-name target set.
...
| ncompress | 4.2.4.4-5 | DIVERGES | IDENTICAL | **IDENTICAL** |
...
| splint | 3.1.2.dfsg1-2 | DIVERGES | pending | pending |
...
| uncrustify | 0.59-2 | DIVERGES | IDENTICAL | **IDENTICAL** |
...
| xdelta | 1.1.3-9 | DIVERGES | IDENTICAL | **IDENTICAL** |
| zip | 3.0-6 | DIVERGES | IDENTICAL | **IDENTICAL** |

**58/58 packages diverge natively across two build roots. 52/52 measured in the corrected arm are byte-identical under `hermit run --strict --no-rcb-time`.**
