...
def selfdet(paths, policy):
    ...
    return len(runs), len(pairs), bad, len(runs[0])
...
def main() -> int:
    ...
    for cell in cells:
        ...
        Z = len(gold_raw)
        g_runs, g_pairs, g_bad, _ = selfdet(gp, "raw")
        for be in BACKENDS:
            population += 1
            ...
            row = dict(cell=cell, backend=be, corpus=CORPUS[cell][0],
                       ...
                       golden_selfdet=f"{g_bad}/{g_pairs} pairs differing over {g_runs} runs",
            ...
            if not cp:
                ...
                continue
            E = len(load(cp[0], "raw"))
            c_runs, c_pairs, c_bad, _ = selfdet(cp, "raw")
            row.update(Z=Z, E=E,
                       selfdet=f"{c_bad}/{c_pairs} pairs differing over {c_runs} runs",
            ...
            if (cell, be) in SELFDET_OVERRIDE:
