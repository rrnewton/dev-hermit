// mmap-heavy determinism probe.
// Exercises: many anonymous mmaps of varied sizes, page touch with a
// deterministic pattern, mprotect toggling, mremap grow/shrink, MAP_SHARED
// anonymous region, msync, munmap, and a final checksum. Output deterministic.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
int main(void) {
    ...
    for (int i = 1; i <= 64; i++) {
        ...
        // toggle protection RO then back RW
        mprotect(m, len, PROT_READ);
        h = fnv1a(m, len, h);
        mprotect(m, len, PROT_READ | PROT_WRITE);
        munmap(m, len);
    }

    // B: mremap grow then shrink.
    size_t base = 4 * (size_t)pg;
    uint8_t *r = mmap(NULL, base, PROT_READ | PROT_WRITE,
                      MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (r == MAP_FAILED) { perror("mmap-r"); return 1; }
    memset(r, 0xab, base);
    uint8_t *r2 = mremap(r, base, base * 4, MREMAP_MAYMOVE);
    if (r2 == MAP_FAILED) { perror("mremap-grow"); return 1; }
    memset(r2 + base, 0xcd, base * 3);
    h = fnv1a(r2, base * 4, h);
    uint8_t *r3 = mremap(r2, base * 4, base, MREMAP_MAYMOVE);
    if (r3 == MAP_FAILED) { perror("mremap-shrink"); return 1; }
    h = fnv1a(r3, base, h);
    munmap(r3, base);
    ...
    for (size_t off = 0; off < slen; off++) s[off] = (uint8_t)(off * 7 + 3);
    msync(s, slen, MS_SYNC);
    h = fnv1a(s, slen, h);
    munmap(s, slen);
