    ...
    match value {
        toml::Value::Table(table) => {
            if let Some(git) = table.get("git").and_then(toml::Value::as_str) {
                if is_expected_reverie_source(git)? {
                    let rev = table
                        ...
                        .ok_or_else(|| {
                            format!(
                                "{path} has a semantic Reverie git dependency without a pinned rev"
                            )
                        })?;
            ...
            for nested in table.values() {
                collect_manifest_pins(nested, path, pins, occurrences)?;
            }
        }
...
fn collect_lock_pins(
    ...
    match value {
        toml::Value::Table(table) => {
            if let Some(source) = table.get("source").and_then(toml::Value::as_str) {
                if let Some(rev) = lock_source_rev(source)? {
                    record_pin(path, rev, pins, occurrences)?;
                }
            }
            for nested in table.values() {
                collect_lock_pins(nested, path, pins, occurrences)?;
            }
        }
        toml::Value::Array(array) => {
            for nested in array {
...
mod tests {
    ...
    #[test]
    fn cargo_lock_source_is_parsed_semantically() {
        let pin = "f".repeat(40);
        let lock: toml::Value = toml::from_str(&format!(
            "version=3\n[[package]]\nname=\"reverie-core\"\nversion=\"0.1.0\"\nsource=\"git+https://github.com/rrnewton/reverie?rev={pin}#{pin}\"\n"
        ))
        .unwrap();
        let mut pins = BTreeSet::new();
        let mut occurrences = 0;
        collect_lock_pins(&lock, "Cargo.lock", &mut pins, &mut occurrences).unwrap();
        assert_eq!(occurrences, 1);
        assert_eq!(pins.into_iter().collect::<Vec<_>>(), vec![pin]);
    }
