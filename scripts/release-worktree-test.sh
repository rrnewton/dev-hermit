...
run_release() {
  local slot=$1
  shift
  (cd "$fixture_root" && env "$@" "$script_dir/release-worktree.rs" --slot "$slot" --clean) \
    >"$output" 2>&1
}

run_release_force() {
  local slot=$1
  (cd "$fixture_root" && "$script_dir/release-worktree.rs" \
...
# adapter after the preliminary statuses. The final removal boundary must see
# it and refuse before invoking Git-level force.
...
echo 'release-worktree-test: PASS (1 recursively published 3-repository target removed; 10 unsafe/unbound targets refused; unrelated path+worktree survived all 11 fixtures)'
