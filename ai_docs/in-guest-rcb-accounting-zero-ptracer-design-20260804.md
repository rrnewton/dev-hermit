...
- **Read #1 = baseline `B`:** first act on entering the shared trampoline dispatch, BEFORE any
  handler branch executes. VERIFIED anchor: immediately before the `drive_tool_syscall` invocation
  at `reverie-preload/src/tool_host.rs:222` (main `8688189a`) — this is the single shared entry both
  liteinst and e9patch route through post-#373.
