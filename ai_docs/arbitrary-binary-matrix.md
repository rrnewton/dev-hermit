...
| Python | site Python `print` | HANG | HANG | BLOCKED | Uses unsupported `CLONE_VFORK`; issue #15. |
...
| VCS | site Git `--version` | HANG | HANG | BLOCKED | Uses unsupported `CLONE_VFORK`; issue #15. |
...
The tested base does not contain the open `CLONE_VFORK` implementation in [PR #27](https://github.com/rrnewton/hermit/pull/27). Make, Cargo builds, site Python, and site Git should be retested after that change lands.
