...
| **Published validation receipt:** immutable receipt commit and path on the receipt branch, branch ancestry, recomputed receipt and log digests, exact repo/head, a counted qualifying ledger row, and (for Hermit schema 6+) the exact resolved Reverie conditions. | Producer: `ci-hub/validation/publish_receipt.py`. Consumer: the parent-pinned `ci-hub/validation/verify_receipt.sh`, which delegates ledger semantics to `ci-hub validate-status`; never PR-controlled verifier code. | **PARTIAL.** The consumer fixture accepts 2/2 legitimate counted same-tip receipts and refuses nonexistent/tampered/zero-execution/incomplete-coverage/missing-binding/moved-tip controls. The minimal safe lander still must dereference this receipt only for the local leg and atomically bind source, replay base, and result; the legacy server-side lander is fail-closed until that extraction lands. |
...
| **Live dependency currency:** the exact Hermit commit's semantic Cargo dependency/source entries identify `rrnewton/reverie`, every manifest and lockfile pin is one lowercase full SHA, and that SHA equals one freshly resolved canonical main ref. Schema-6 receipts carry `{repository, ref, pinned_sha, resolved_sha}` rather than a copied boolean. | Parent authority: `ci-hub reverie-pin-status` / `ci-hub/lib/reverie_pin.rs`; local-receipt consumers share `ci-hub/lib/qualifying_receipt.rs`. It parses tracked TOML from the exact commit object, so comments cannot authorize and valid URLs with or without `.git` are recognized. Hermit's hosted gate separately calls `scripts/check-reverie-pin.rs`. | **PARTIAL.** Parent brackets cover same-tip acceptance; moved-tip, comment spoof, malformed/missing/mixed/tampered binding refusal; and cache invalidation on a ref move. Local status/history/publisher/verifier paths call the parent authority. Remaining holes are load-bearing: Hermit's separate checker must adopt the same semantic parser (its line parser has the old comment/no-`.git` blind spot), and the minimal safe lander must atomically bind source X, observed main base Y, replay result Z, and a final-boundary ref refresh. Legacy `land-pr.sh` is fail-closed rather than claiming those conditions. |
