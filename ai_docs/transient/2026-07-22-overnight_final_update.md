...
- **Final 100-run stress test: 90/100 pass (10% flake), and every failure is
  ...
  (site Python wrapper, node). The 8 single/light-threaded apps were a perfect 80/80.
...
| **Git** | **stock /usr/bin/git**: init+config+add+commit+log, branch/checkout/3-way-merge/diff — PASS, **commit hashes deterministic** (build-reproducibility win). site Git 2.53 = slow/timeout (use stock) |
...
| **python3 (=site Python wrapper)** | **1** | **9** | `FFFF.FFFFF` |
| sqlite3 SELECT | 10 | 0 | `..........` |
| **node -e** | **9** | **1** | `...F......` |
| lua -e | 10 | 0 | `..........` |
| curl --version | 10 | 0 | `..........` |
...
- Both failing apps are **multithreaded runtimes reading wall-clock time from worker
  threads**: site Python wrapper (heavy MT startup + folly telemetry) and node (V8 background
...
- Stock `/usr/bin/python3.9` is deterministic; the PATH `python3`=site Python wrapper is the outlier.
...
1. **Multithreaded wall-clock time** — python threading / site Python wrapper / node fail L2 via
...
site Python wrapper & Meta-git (heavyweight wrappers — use stock `/usr/bin/python3` and
