...
| site Git wrapper | `<site-git> --version` | FAIL | NOT MEASURED | Runtime/telemetry threads diverge; stock Git is the established control. |
...
| site Python version | `python3 --version` | PASS L2 | FAIL | Recorder panics on ioctl `0x8946`; guest output itself is correct. |
| stock Python JSON | `/usr/bin/python3.9 -c 'import json; ...'` | PASS L2 | NOT MEASURED | Stock runtime control passes. |
| site Python JSON/CSV | `python3 -c ...` | FAIL | NOT MEASURED | Helper-thread/RNG startup scheduling diverges. |
...
1. **Replay completeness trails strict execution.** Concurrent real programs
   such as Podman, crun, rustc, site Python, raw Git workflows, and Node expose
...
5. **Binary provenance matters.** Stock `/usr/bin/git` passes L2 while the Meta
   ...
   site runtime can fail during startup.

## Evidence sources

- `ai_docs/transient/2026-07-24-strict-compat-matrix.md`, batches 1-59 and post-envp R/R
  batches 1-15.
