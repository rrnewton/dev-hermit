...
- **NEVER DOUBLE-DISPATCH A HOSTED WORKFLOW AT ONE SHA — it poisons that SHA's authority.** Two concurrent dispatches contend for admission-limited runners; some jobs of one run get starved and cancelled, and a cancelled job **writes a `failure` check under the authority job's name**, so `hosted-status` reports RED for a head whose genuine result was green. Measured on hermit#1910 at `fda945d7` and hermit#1911 at `18758e9a`: in the second case 30+ jobs succeeded and exactly two (`test: sabre`, `e2e: c-programs__verify__ptrace`) were cancelled, which was enough to flip `Regular tests (GitHub-managed portable)` to `failure`. **Check for an existing queued/running dispatch before dispatching** (`gh run list --workflow <id> --branch <br> --json headSha,status`). Cancelling the extra does not help and is not what causes the red — the contention does; a manual cancel merely produces the same result sooner. Once a SHA is poisoned, the repair is **one** clean dispatch, so the newest instance of the authority job is green: verified to move `fda945d7` from `HOSTED RED 0/1` to `HOSTED GREEN 1/1`.
