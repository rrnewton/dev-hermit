...
- **A run's conclusion is not the registered job's conclusion, and NEVER CANCEL A DUPLICATE HOSTED RUN.** `gh run view <id> --json conclusion` describes a *workflow run*; the exact-head authority is the named *job* (`ci-hub hosted-status`). They disagree, and the authority wins. Worse, cancelling a run **writes a `failure` check for its authority job at that SHA**, so the tidy-minded act of cancelling a redundant dispatch can turn a green head red and block its own gate. Observed 2026-08-07 on hermit#1910 at `fda945d7`: run `31189422166` had `Regular tests (GitHub-managed portable): success`, a duplicate dispatch was cancelled to save CI capacity, and `hosted-status` then reported `HOSTED RED … positive=0/1` for a head whose real result was green. **Check for an existing queued/running dispatch BEFORE dispatching** (`gh run list --workflow <id> --json headSha,status`); if you have already double-dispatched, let both finish rather than cancelling either.
