...
fn rejects_unsupported_or_unsound_shapes() {
    ...
    const CASES: &[(&str, &str)] = &[
        ...
        (
            "reference",
            "&'static case_reference::Target: PodValue",
        ),
    ...
    let macro_root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = macro_root.parent().expect("workspace root");
    ...
    let manifest = format!(
        "[workspace]\n\
         ...
         edition = \"2024\"\n\
         [dependencies]\n\
         shmem-pod = {{ path = {:?}, default-features = false }}\n\
         shmem-pod-macros = {{ path = {:?} }}\n",
        workspace_root.join("pod-v2-types"),
        macro_root,
    );
    ...
    for (case, _) in CASES {
        let path = macro_root.join("tests/ui").join(format!("{case}.rs"));
