...
fn run_parent(mut options: ParentOptions) -> Result<(), Box<dyn Error>> {
    ...
    let result = (|| -> Result<(), Box<dyn Error>> {
        ...
        println!(
            "v2-ok artifact={} processes={} threads_per_process={} iterations={} calls={} snzi_cycles={} snzi_leaves={} snzi_query=false snzi_quiescent=true entries={} capacity={} allocated={} layout=size:{}/align:{}/hash:{:016x} code_va=0x{:x} state_va=0x{:x} payload_va=0x{:x}",
            artifact.digest_hex(),
            options.workers + 1,
            options.threads,
            options.iterations,
...
fn finish_with_cleanup<T>(
    ...
    match (primary, cleanup) {
        ...
        (Err(primary), Err(cleanup)) => {
            eprintln!("pod-v2-host: cleanup after failure also failed: {cleanup}");
            Err(primary)
        }
    }
}
