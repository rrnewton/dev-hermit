...
fn main() {
    if let Err(error) = run() {
        eprintln!("shmem-pod-image-compiler: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "emitted {}-byte pod image ({} linked code bytes, sha256 {})",
        image.len(),
        pod_bytes.len(),
        hex(&image_hash)
    );
...
fn audit_linked_elf<'a>(
    ...
    validate_pod_alignment(pod.align())?;
...
fn validate_pod_alignment(alignment: u64) -> Result<(), Box<dyn Error>> {
    if alignment == 0 || !alignment.is_power_of_two() || alignment > PAGE_SIZE as u64 {
        return Err(format!(
            ".pod alignment {alignment} is not a power of two within the runtime's {PAGE_SIZE}-byte mapping alignment"
        )
        .into());
    }
    Ok(())
...
fn inspect_sdk(options: &Options) -> Result<SdkMetadata, Box<dyn Error>> {
    ...
    let canonical_manifest = options.sdk_manifest.canonicalize()?;
...
fn sdk_rustc_args(sdk: &SdkMetadata) -> String {
    format!(
        "--crate-name shmem_pod --edition={} --crate-type=rlib --emit=link --cfg feature=\"linux-futex\" {}",
        sdk.edition,
...
fn pod_rustc_args(options: &Options) -> String {
    format!(
        "--crate-name shmem_pod_image_code --edition=2024 --crate-type=lib --emit=obj --extern shmem_pod={} {}",
        options.sdk_rlib.display(),
        COMMON_CODEGEN_ARGS.join(" ")
    )
}
...
fn parse_options() -> Result<Options, Box<dyn Error>> {
    ...
    while let Some(argument) = args.next() {
        ...
        match argument.as_str() {
            ...
            "-h" | "--help" => {
                println!(
                    "usage: shmem-pod-image-compiler --source FILE --sdk-manifest FILE --sdk-source FILE --sdk-rlib FILE --linker-script FILE --output FILE --object FILE --elf FILE --manifest FILE [--rustc PATH]"
                );
                std::process::exit(0);
            }
...
mod tests {
    ...
    #[test]
    fn rejects_alignment_stronger_than_the_runtime_mapping() {
        validate_pod_alignment(1).unwrap();
        validate_pod_alignment(64).unwrap();
        validate_pod_alignment(PAGE_SIZE as u64).unwrap();
        assert!(validate_pod_alignment(0).is_err());
        assert!(validate_pod_alignment(3).is_err());
        assert!(validate_pod_alignment((PAGE_SIZE * 2) as u64).is_err());
    }
    ...
    fn dep_info_snapshot_parses_escaped_paths_and_detects_changes() {
        ...
        let directory =
            env::temp_dir().join(format!("shmem-pod-dep-info-{}-{id}", std::process::id()));
